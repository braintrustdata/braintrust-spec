...
### Spec fetch implementation

Download and extract the tarball from GitHub using whatever tools your language provides.  The URL pattern is:

```
https://github.com/braintrustdata/braintrust-spec/archive/<ref>.tar.gz
```

Strip the top-level directory from the archive (e.g. `braintrust-spec-af0e006/`) when extracting so that the cache directory contains `test/llm_span/` directly.

Use your language's standard HTTP and archive libraries rather than shelling out to `curl`/`bash` — this ensures the fetch works on all platforms including Windows.
